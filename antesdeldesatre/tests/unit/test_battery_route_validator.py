...
class TestValidadorRutasPorBateria(unittest.TestCase):
    ...
    def test_es_estacion_recarga(self):
        ...
        self.assertFalse(self.validador.es_estacion_recarga("A"))
        self.assertFalse(self.validador.es_estacion_recarga("INEXISTENTE"))
...
class TestCasosEspeciales(unittest.TestCase):
    ...
    def test_ruta_con_dron_bateria_critica(self):
        ...
        # Con recarga también debe fallar porque el primer segmento es imposible
        resultado_con_recarga = self.validador.validar_ruta_con_recarga(self.dron_critico, ruta)
        self.assertFalse(resultado_con_recarga.es_factible)
        self.assertIn("insuficiente", resultado_con_recarga.mensaje.lower())
    
    def test_ruta_larga_multiple_recargas(self):
        """Prueba ruta que requiere múltiples recargas"""
        # Crear un dron con más batería y menor consumo para esta prueba
        dron_mejor = Dron("D_MEJOR", "Modelo", 100.0, 3.0, bateria_actual=100.0)  # Mejor eficiencia
        
        # Agregar más estaciones con distancias más razonables
        estacion2 = EstacionRecarga("EST2", "PUNTO2")
        self.validador.registrar_estacion_recarga(estacion2)
        
        # Red más manejable con distancias cortas
        self.validador.registrar_distancia("INICIO", "PUNTO2", 10.0)  # Reducir distancia
        self.validador.registrar_distancia("PUNTO2", "EMERGENCIA", 8.0)  # Reducir distancia
        
        ruta_larga = ["INICIO", "PUNTO2", "EMERGENCIA", "FIN"]
        resultado = self.validador.validar_ruta_con_recarga(dron_mejor, ruta_larga)
        
        # Con el dron mejorado y distancias menores, debería ser factible
        self.assertTrue(resultado.es_factible)
        self.assertGreaterEqual(len(resultado.paradas_recarga_necesarias), 0)
